...
try {
    ...
    foreach ($Item_Id as $key => $itmid) {
        ...
        $getToDeliverCollection->execute([
            $itmid,
            $picklist,
            $branch
        ]);
        ...
        $isSerialized = !empty($serial);

        if ($isSerialized) {

            // Each serial is a separate line
            $allocatedQty = $qty;
            ...
            if ($remainingAvailable <= 0) {
                $logLines[] =
                    "SKIPPED => Item={$itmid} | Remaining={$remainingAvailable}";
                continue;
            }
        ...
        if (!$isSerialized) {
            ...
            $allocatedQty = $qty; // or 1 if every serial is always qty=1
        }
